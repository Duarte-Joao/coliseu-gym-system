...
<div class="lay">
  ...
  <aside class="sb">
    ...
    <div class="sb-footer">
      ...
      <div class="user-pill">
        <div class="avatar">{{ strtoupper(substr(auth()->user()->name ?? 'IN', 0, 2)) }}</div>
        <div class="user-info">
          <strong>{{ auth()->user()->name ?? 'Instrutor' }}</strong>
          <small>{{ $instrutor->especialidade ?? '—' }} · Turno {{ $instrutor->turno ?? '—' }}</small>
        </div>
      </div>
  ...
  <main class="main">
    ...
    <section class="sec active" id="sec-visao">
      ...
      <div class="stats">
        <div class="stat"><label>Especialidade</label><div class="val purple-c">{{ $instrutor->especialidade ?? '—' }}</div><sub>Principal</sub></div>
        <div class="stat gold"><label>Carga Horária</label><div class="val gold-c">{{ $instrutor->carga_hora ?? '—' }}</div><sub>Por dia</sub></div>
        <div class="stat amber"><label>Turno</label><div class="val amber-c">{{ $instrutor->turno ?? '—' }}</div><sub>Designado</sub></div>
        <div class="stat"><label>Alunos sob tutela</label><div class="val purple-c">{{ $totalAlunos }}</div><sub>{{ $semFicha }} sem ficha ativa</sub></div>
      </div>
