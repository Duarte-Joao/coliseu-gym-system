      ...
      <form id="form-login" action="{{ route('login.post') }}" method="POST">
        ...
        <div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:8px;padding:1rem;margin-bottom:1.5rem;color:#f87171;">
            ...
            <ul style="margin-top:0.5rem;padding-left:1.5rem;">
                @foreach($errors->all() as $error)<li style="margin:0.25rem 0">{{ $error }}</li>@endforeach
            </ul>
        </div>
        @endif
...
<script>
  ...
  function setFormMode(acao) {
    ...
    loginInputs.forEach(input => { if (input.type !== 'hidden') input.disabled = !isLoginMode; });
    cadastroInputs.forEach(input => { if (input.type !== 'hidden') input.disabled = isLoginMode; });
  }
